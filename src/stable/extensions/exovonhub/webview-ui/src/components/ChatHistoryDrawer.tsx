...
export const ChatHistoryDrawer: React.FC<ChatHistoryDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/60 backdrop-blur-sm transition-opacity duration-200" onClick={onClose}>
      {/* OBSIDIAN DARK GLASS DRAWER */}
      <div
        role="dialog"
        aria-modal="true"
        aria-label="Chat History"
        className="w-80 max-w-[88vw] h-full bg-[#121217]/95 border-l border-white/10 shadow-2xl flex flex-col backdrop-blur-2xl relative z-10 select-none animate-slide-in-right"
        onClick={(e) => e.stopPropagation()}
      >
        {/* HEADER */}
        <div className="px-4 py-3.5 border-b border-white/10 flex items-center justify-between relative z-10 bg-white/[0.02]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                <h2 className="text-xs font-semibold text-zinc-100 tracking-wide font-sans">Chat History</h2>
                <span className="px-1.5 py-0.5 rounded-md bg-white/5 border border-white/10 text-[10px] font-mono text-zinc-400">
                  {chatThreads.length}
                </span>
              </div>
              <p className="text-[10px] text-zinc-500">Past sessions & threads</p>
            </div>
          </div>
        ...
        {/* ACTIONS & SEARCH BAR */}
        <div className="p-3 border-b border-white/5 space-y-2.5 relative z-10 bg-black/20">
          {/* NEW CHAT BUTTON */}
          <button
            onClick={() => {
              onNewChat();
            ...
            className="w-full flex items-center justify-between px-3 py-2 bg-white/10 hover:bg-white/15 text-white rounded-lg font-medium text-xs border border-white/15 transition-all shadow-sm active:scale-[0.99] group cursor-pointer"
          >
            ...
            <span className="text-[10px] font-mono text-zinc-400 bg-black/40 px-1.5 py-0.5 rounded border border-white/10">
              Ctrl+N
            </span>
          </button>
          ...
          <div className="relative">
            <svg className="w-3.5 h-3.5 absolute left-2.5 top-1/2 -translate-y-1/2 text-zinc-500 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <input
              ...
              className="w-full pl-8 pr-7 py-1.5 bg-zinc-950/70 border border-white/10 focus:border-white/20 rounded-lg text-xs text-zinc-200 placeholder-zinc-500 focus:outline-none transition-all font-sans"
            />
            {searchQuery && (
              <button
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-4 relative z-10 scrollbar-thin">
                    ...
                    <div
                      ...
                      className={`group relative rounded-lg border transition-all duration-150 overflow-hidden ${
                        isActive
                          ...
                          : 'bg-zinc-950/40 hover:bg-white/5 border-white/5 hover:border-white/10'
                      }`}
                    >
                      {/* ACTIVE PILL ACCENT */}
                      {isActive && (
                        <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-white/80 rounded-l" />
                      )}

                      {/* INLINE DELETE CONFIRMATION STATE */}
                      {isConfirmingDelete ? (
                        <div className="p-2.5 flex items-center justify-between bg-rose-950/50 border border-rose-800/40 rounded-lg">
                        ...
                        /* INLINE RENAME STATE */
                        <div className="p-2 flex items-center gap-1.5 bg-zinc-950/90 border border-white/20 rounded-lg">
                          <input
                            ref={editInputRef}
                            type="text"
                            value={editTitle}
                            ...
                            className="flex-1 bg-black/50 px-2 py-1 border border-white/10 rounded text-xs text-white focus:outline-none focus:border-white/30"
                          />
                          <button
                            onClick={() => handleSaveRename(thread.id)}
                            className="p-1 text-zinc-300 hover:text-white hover:bg-white/10 rounded cursor-pointer"
                            title="Save title"
                          >
                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        >
                          <div className="flex items-center justify-between gap-1.5">
                            {/* TITLE */}
                            <span
                              className={`text-xs font-medium truncate flex-1 font-sans ${
                                isActive ? 'text-white font-semibold' : 'text-zinc-200 group-hover:text-white'
                              }`}
                            ...
                            <div
                              className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity bg-zinc-900/95 px-1 py-0.5 rounded-md border border-white/10 shadow-sm shrink-0"
                              onClick={(e) => e.stopPropagation()}
                            >
                              {/* RENAME */}
                              {onRenameThread && (
                          ...
                          {/* METADATA FOOTER */}
                          <div className="flex items-center justify-between text-[10px] text-zinc-500 font-mono mt-0.5 pt-0.5">
          ...
          {chatThreads.length > 0 && filteredThreads.length === 0 && (
            <div className="p-6 text-center space-y-2">
              <div className="w-10 h-10 mx-auto rounded-full bg-zinc-950/70 border border-white/10 flex items-center justify-center text-zinc-400">
              ...
              <button
                onClick={() => setSearchQuery('')}
                className="mt-2 text-xs text-zinc-400 hover:text-white underline font-medium cursor-pointer"
              >
                Clear Search
              </button>
            </div>
          ...
          {chatThreads.length === 0 && (
            <div className="p-6 text-center space-y-3">
              <div className="w-12 h-12 mx-auto rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-zinc-400">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-zinc-200 text-zinc-950 rounded-lg text-xs font-semibold shadow-sm transition-all cursor-pointer"
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" />
                </svg>
        ...
        {showClearAllConfirm && (
          <div className="p-3 border-t border-rose-800/40 bg-zinc-950/95 backdrop-blur-md space-y-2 animate-slide-up">
